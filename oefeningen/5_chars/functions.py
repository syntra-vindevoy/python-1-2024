            ...
            for k in range(j + 1, length - 2):
                ...
                for l in range(k + 1, length - 1):
                    ...
                    for m in range(l + 1, length):
                        ...
                        character_combinations.add(
                            "".join([char_1, char_2, char_3, char_4, char_5])
                        )
        ...
        for j in range(i + 1, length - 3):
            ...
            for k in range(j + 1, length - 2):
                ...
                for l in range(k + 1, length - 1):
                    char_4 = character_stringlist[l]
                    character_combinations.add(
                        "".join([char_1, char_2, char_3, char_4])
                    )
                    for m in range(l + 1, length):
                        char_5 = character_stringlist[m]
                        character_combinations.add(
...
def get_matching_dict_adding(chars, char_combination_length, word_set):
    char_combination_set = get_char_combination_set(chars, char_combination_length)
    combination_dict = {comb: 0 for comb in char_combination_set}
    word_dict = {word: char_combination_set.copy() for word in word_set}
    for char in chars:
        word_matched_list = [word for word in word_set if char in word]
    ...
    for comb in combination_dict:
        combination_dict[comb] = sum(
            1 for word in word_dict if comb not in word_dict[word]
        )
