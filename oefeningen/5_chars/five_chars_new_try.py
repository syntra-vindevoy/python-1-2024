...
def get_combination_set(character_stringlist):
    ...
    for i in range(0,len_alphabet):
        char_1 = alphabet[i]
        for j in range (i+1,len_alphabet):
            char_2 = alphabet[j]
            for k in range (j+1,len_alphabet):
                char_3 = alphabet[k]
                for l in range (k+1,len_alphabet):
                    char_4 = alphabet[l]
                    for m in range (l+1,len_alphabet):
                        char_5 = alphabet[m]
...
def main():
    ...
    combination_original_set = get_combination_set(char_list)
    combination_dict = get_dict_from_set(combination_original_set)

    
    # test variables, mark as comment to  proceed
    
    ic.enable()
    char_list = 'abc'
    combination_original_set = {'ab', 'ac', 'bc'}
    ic(combination_original_set)
    combination_dict = {'ab': 0, 'ac': 0, 'bc': 0}
    ic(combination_dict)
    word_list = ['aboriginal',
                 'abc',
                 'citroen',
                 'aha',
                 'pompidom', 
    ...
    for char_index in range(0, len(char_list)): 
        combination_set = combination_original_set.copy()
        matching_words = get_matching_items_from_set(char_list[char_index], word_list) 
        # ic(matching_words)
        matching_combinations = get_matching_items_from_set(char_list[char_index], combination_set)
        ...
        for char_sub_index in range(char_index+1, len(char_list)):
            ...
            sub_matching_combinations = \
                get_matching_items_from_set(char_list[char_sub_index], matching_combinations)
            sub_matching_words = \
                get_matching_items_from_set(char_list[char_sub_index], matching_words)
            
            for sub_matching_combination in sub_matching_combinations:
                combination_dict[sub_matching_combination] -= len(sub_matching_words)
        ic(combination_dict)
    '''
