...
def count_words_with_chars(word_list, comb):
    """
    Counts the number of words in word_list that contain any character from comb.

    :param word_list: List of words to check.
    :param comb: String of characters to look for in the words.
    :return: Count of words containing any character from comb.
    """
    count = 0
    for word in word_list:
        for char in comb:
            if char in word:
                count += 1
                break  # Stop checking other characters in comb for this word
    return count
...
def main():
    ...
    char_list_sorted = sorted(
        char_count_dict.items(), key=lambda item: item[1], reverse=True
    )

    best_estimate = "".join([char for char, _ in char_list_sorted[-5:]])
    best_estimate_count = sum(
        1 for word in word_list if any(char in word for char in best_estimate)
    )
    ...
    for i in range(0, length - 4):
        ...
        for j in range(i + 1, length - 3):
            ...
            for k in range(j + 1, length - 2):
                ...
                count = sum(
                    1 for word in word_list if any(char in word for char in comb)
                )
                if count > best_estimate_count:
                    continue
                # reduced_dict[comb] = count
                ...
                for l in range(k + 1, length - 1):
                    ...
                    for m in range(l + 1, length):
                        ...
                        count = sum(
                            1
                            for word in word_list
                            if any(char in word for char in comb)
                        )
                        if count > best_estimate_count:
                            continue
                        # print(f"{comb}:{count}")
