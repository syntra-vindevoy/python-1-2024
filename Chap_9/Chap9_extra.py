...
def number_seacher(target_number, search_min, search_max, counter=0):
    counter += 1
    ...
    if search == target_number:
        ...
        return number_seacher(target_number, search + 1, search_max, counter)
    else:
        #print(f"Step {counter}: {search} is higher than {target_number}")
        return number_seacher(target_number, search_min, search - 1, counter)


# Input and function call
max_number = 10000
target_number = #int(input("Enter the target number (1 to 10000): "))
final_value = number_seacher(target_number, 1, max_number)
